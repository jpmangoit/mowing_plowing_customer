...
class _UploadPhotosState extends State<UploadPhotos> {
  ...
  void selectImages() async {
    ...
    if (selectedImages.isNotEmpty) {
      ...
      await localStorage.setString('image0L', imageFileList![0].path);
      image1 = imageFileList!.length > 1 ? imageFileList![1].path : null;
      imageFileList!.length > 1
          ? await localStorage.setString('image1L', imageFileList![1].path)
          : null;
      image2 = imageFileList!.length > 2 ? imageFileList![2].path : null;
      imageFileList!.length > 2
          ? await localStorage.setString('image2L', imageFileList![2].path)
          : null;
      image3 = imageFileList!.length > 3 ? imageFileList![3].path : null;
      imageFileList!.length > 3
          ? await localStorage.setString('image3L', imageFileList![3].path)
          : null;
    }
    if (mounted) {
      setState(() {});
  ...
  Future showImages() async {
    SharedPreferences localStorage = await SharedPreferences.getInstance();
    image0 = localStorage.getString("image0L");
    image1 = localStorage.getString("image1L");
    image2 = localStorage.getString("image2L");
    image3 = localStorage.getString("image3L");
    if (localStorage.getString('instructionL') == null) {
      instructionController = TextEditingController();
      await localStorage.setString('instructionL', instructionController.text);
    } else {
      var instructionText = localStorage.getString('instructionL');
