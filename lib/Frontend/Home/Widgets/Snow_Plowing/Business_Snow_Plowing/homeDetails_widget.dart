...
class HomeDetailsBusiness extends StatefulWidget {
  const HomeDetailsBusiness({super.key});

  @override
  State<HomeDetailsBusiness> createState() => _HomeDetailsBusinessState();
}

class _HomeDetailsBusinessState extends State<HomeDetailsBusiness> {
  // ------------------------
  ...
  inc() async {
    ...
    if (width == 50) {
      ...
      await localStorage.setInt('widthB', width!);
    } else {
      if (mounted) {
        setState(() {
          width = width! + 1;
  ...
  incL() async {
    ...
    if (length == 50) {
      ...
      await localStorage.setInt('lengthB', length!);
    } else {
      if (mounted) {
        setState(() {
          length = length! + 1;
  ...
  Future getSnowPlowingCatPrSch() async {
    ...
    if (response["message"] == "Unauthenticated.") {
      ...
      if (response["success"]) {
        ...
        await localStorage.setString('snowPlowingSchedulesB',
            jsonEncode(response["data"]["snowPlowingSchedules"]));

        int? w = localStorage.getInt('widthB');
        int? l = localStorage.getInt('lengthB');
        bool? side = localStorage.getBool('sidewalkB');
        bool? walk = localStorage.getBool('walkawayB');
        String? Side = localStorage.getString('SidewalkB');
        String? Walk = localStorage.getString('WalkwayB');
        String? snowDepInVal = localStorage.getString('snowDepthInitValueB');

        w == null ? width = 1 : width = w;
        w == null ? await localStorage.setInt('widthB', 1) : null;
        l == null ? length = 1 : length = l;
        l == null ? await localStorage.setInt('lengthB', 1) : null;

        side == null ? sidewalk = false : sidewalk = side;
        side == null ? await localStorage.setBool('sidewalkB', false) : null;
        walk == null ? walkaway = false : walkaway = walk;
        walk == null ? await localStorage.setBool('walkawayB', false) : null;

        Side == null ? groupVal = "small" : groupVal = Side;
        Side == null
            ? await localStorage.setString('SidewalkB', "small")
            : null;
        Walk == null ? groupValW = "small" : groupValW = Walk;
        Walk == null ? await localStorage.setString('WalkwayB', "small") : null;
        if (snowDepInVal == null) {
          snowDepthInitValue = jsonResponse[0]["name"].toString();
          await localStorage.setString(
              'snowDepthInitValueIdB', jsonResponse[0]["id"].toString());
        } else {
                      ...
                      Material(
                        ...
                        child: DropdownButtonFormField(
                          ...
                          onChanged: (newValue) async {
                            ...
                            await localStorage.setString('snowDepthInitValueB',
