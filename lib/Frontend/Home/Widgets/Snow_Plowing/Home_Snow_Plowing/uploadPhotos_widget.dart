...
class UploadPhotosHome extends StatefulWidget {
  const UploadPhotosHome({super.key});

  @override
  State<UploadPhotosHome> createState() => _UploadPhotosHomeState();
}

class _UploadPhotosHomeState extends State<UploadPhotosHome> {
  bool loading = true;
  ...
  void selectImages() async {
    ...
    if (selectedImages.isNotEmpty) {
      ...
      await localStorage.setString('image0H', imageFileList![0].path);
      image1 = imageFileList!.length > 1 ? imageFileList![1].path : null;
      imageFileList!.length > 1
          ? await localStorage.setString('image1H', imageFileList![1].path)
          : null;
      image2 = imageFileList!.length > 2 ? imageFileList![2].path : null;
      imageFileList!.length > 2
          ? await localStorage.setString('image2H', imageFileList![2].path)
          : null;
      image3 = imageFileList!.length > 3 ? imageFileList![3].path : null;
      imageFileList!.length > 3
          ? await localStorage.setString('image3H', imageFileList![3].path)
          : null;
    }
    if (mounted) {
      setState(() {});
  ...
  Future showImages() async {
    SharedPreferences localStorage = await SharedPreferences.getInstance();
    image0 = localStorage.getString("image0H");
    image1 = localStorage.getString("image1H");
    image2 = localStorage.getString("image2H");
    image3 = localStorage.getString("image3H");
    if (localStorage.getString('instructionH') == null) {
      instructionController = TextEditingController();
      await localStorage.setString('instructionH', instructionController.text);
    } else {
      var instructionText = localStorage.getString('instructionH');
