...
class ServiceSummaryHome extends StatefulWidget {
  const ServiceSummaryHome({
    super.key,
  });

  @override
  State<ServiceSummaryHome> createState() => _ServiceSummaryHomeState();
}

class _ServiceSummaryHomeState extends State<ServiceSummaryHome> {
  bool loading = true;
  var tenPerc;
  var fifteenPerc;
  var twentyPerc;
  ...
  Future summary() async {
    ...
    List? images = [];
    var image0 = localStorage.getString('image0H');
    var image1 = localStorage.getString('image1H');
    var image2 = localStorage.getString('image2H');
    var image3 = localStorage.getString('image3H');
    images.add(image0 ?? image0);
    images.add(image1 ?? image1);
    images.add(image2 ?? image2);
    images.add(image3 ?? image3);
    var decode =
        jsonDecode(localStorage.getString('snowPlowingScheduleSelectedH')!);
    ...
    var response = await BaseClient().orderSummaryHome(
      ...
      "2",
      "Home",
      jsonDecode(localStorage.getString('snowPlowingScheduleSelectedH')!)["id"]
          .toString(),
      localStorage.getInt('widthH').toString(),
      localStorage.getInt('lengthH').toString(),
      localStorage.getString(
        'snowDepthInitValueIdH',
      )!,
      localStorage.getString('instructionH'),
      localStorage.getBool('sidewalkH').toString(),
      localStorage.getBool('sidewalkH') == true
          ? localStorage.getString('SidewalkH')
          : null,
      localStorage.getBool('walkawayH').toString(),
      localStorage.getBool('walkawayH') == true
          ? localStorage.getString('WalkwayH')
          : null,
      images,
    );
