...
class UploadPhotosCar extends StatefulWidget {
  const UploadPhotosCar({super.key});

  @override
  State<UploadPhotosCar> createState() => _UploadPhotosCarState();
}

class _UploadPhotosCarState extends State<UploadPhotosCar> {
  bool loading = true;
  ...
  void selectImages() async {
    ...
    if (selectedImages.isNotEmpty) {
      ...
      await localStorage.setString('image0C', imageFileList![0].path);
      image1 = imageFileList!.length > 1 ? imageFileList![1].path : null;
      imageFileList!.length > 1
          ? await localStorage.setString('image1C', imageFileList![1].path)
          : null;
      image2 = imageFileList!.length > 2 ? imageFileList![2].path : null;
      imageFileList!.length > 2
          ? await localStorage.setString('image2C', imageFileList![2].path)
          : null;
      image3 = imageFileList!.length > 3 ? imageFileList![3].path : null;
      imageFileList!.length > 3
          ? await localStorage.setString('image3C', imageFileList![3].path)
          : null;
    }
    if (mounted) {
      setState(() {});
  ...
  Future showImages() async {
    SharedPreferences localStorage = await SharedPreferences.getInstance();
    image0 = localStorage.getString("image0C");
    image1 = localStorage.getString("image1C");
    image2 = localStorage.getString("image2C");
    image3 = localStorage.getString("image3C");
    if (localStorage.getString('instructionC') == null) {
      instructionController = TextEditingController();
      await localStorage.setString('instructionC', instructionController.text);
    } else {
      var instructionText = localStorage.getString('instructionC');
